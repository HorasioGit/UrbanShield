...
RAW_DATA_DIR = os.path.join(BASE_DIR, '../data/raw_dataset')
...
def parse_dates_generic(row):
    ...
    dates = []
    for y in ['2017', '2018', '2019', '2020', '2023']:
...
flood_files = [
    ...
    'Filedata Data Kejadian Bencana Banjir di Provinsi DKI Jakarta Tahun 2018.csv',
...
weather_master = weather_master[weather_master['datetime'].dt.year.isin([2017, 2018, 2019, 2020])]

final_abt = pd.merge(weather_master, flood_master, on=['datetime', 'kota_administrasi'], how='left')

# MERGE BOGOR DATA
bogor_path = os.path.join(RAW_DATA_DIR, 'bogor_hulu_meteo.csv')
if os.path.exists(bogor_path):
    bogor_df = pd.read_csv(bogor_path)
    try:
        bogor_df['date'] = pd.to_datetime(bogor_df['date']).dt.tz_convert('Asia/Jakarta').dt.tz_localize(None)
    except AttributeError:
        bogor_df['date'] = pd.to_datetime(bogor_df['date']).dt.tz_localize('UTC').dt.tz_convert('Asia/Jakarta').dt.tz_localize(None)
    bogor_df = bogor_df.rename(columns={'date': 'datetime', 'precipitation': 'bogor_rain'})[['datetime', 'bogor_rain']]
    final_abt = pd.merge(final_abt, bogor_df, on='datetime', how='left')
    final_abt['bogor_rain'] = final_abt['bogor_rain'].fillna(0)
else:
    final_abt['bogor_rain'] = 0
...
cols = ['datetime', 'tanggal', 'jam', 'kota_administrasi', 
        ...
        'precipitation', 'bogor_rain', 'precip_3h_sum', 'precip_6h_sum', 'precip_12h_sum', 'status_banjir']
final_abt = final_abt[[c for c in cols if c in final_abt.columns]]

output_file = os.path.join(BASE_DIR, '../data/UrbanShield_ABT_Final.csv')
